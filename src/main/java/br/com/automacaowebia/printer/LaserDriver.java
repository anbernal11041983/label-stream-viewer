...
/**
 * Driver responsável por enviar quadros STX/ETX ao laser Codiline (JCZ)
 * e validar o ACK (0x02 0x06 … 0x03).
 */
public final class LaserDriver {
     ...
     * ========================================================================= */
    /**
     * Lista os templates, procura por HELLO.ncfm e, se encontrar, executa
     * load ▸ seta ▸ start.  Caso contrário lança IOException.
     */
    ...
    /** Obtém a lista de arquivos salvos na controladora */
    private static List<String> fetchFileList(OutputStream out, InputStream in) throws IOException {
        ...
        int b;
        while ((b = in.read()) != -1) { buf.write(b); if (b == ETX) break; }
        ...
        if (idx == -1) return List.of();        // nada encontrado
    ...
    private static byte[] frame(String payload) {
        ...
        byte xor = 0; for (byte b : data) xor ^= b;
    ...
    private static void readAck(InputStream in, String etapa) throws IOException {
        ByteArrayOutputStream buf = new ByteArrayOutputStream();
        int b; while ((b = in.read()) != -1) { buf.write(b); if (b == ETX) break; }
        byte[] resp = buf.toByteArray();
